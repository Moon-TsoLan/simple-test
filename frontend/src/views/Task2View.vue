...
function setScenario(id: number) { active.value = id; rows.value = []; pairRows.value = []; error.value = '' }

async function run() {
  loading.value = true; error.value = ''; rows.value = []; pairRows.value = []
  try {
    if (active.value === 1) {
...
onMounted(async () => { organizations.value = await api<Organization[]>('/api/task2/organizations?limit=200') })
</script>

<template>
  <section class="scenario-layout">
    <aside class="panel scenario-nav"><span class="eyebrow">FIVE SCENARIOS</span><button v-for="scenario in scenarios" :key="scenario.id" :class="{ active: active === scenario.id }" @click="setScenario(scenario.id)"><b>0{{ scenario.id }}</b><span><strong>{{ scenario.title }}</strong><small>{{ scenario.help }}</small></span></button></aside>
    <div class="scenario-main">
      <article class="panel scenario-query"><span class="eyebrow">SCENARIO 0{{ active }}</span><h2>{{ current.title }}</h2><p>{{ current.help }}。金额口径统一为中标或成交金额。</p>
        <div class="query-row">
      ...
      <EmptyState v-else-if="!loading && !error" :title="organizations.length ? '选择条件后运行分析' : '暂无任务二关系数据'" :description="organizations.length ? '结果会同时呈现为统计表和关系图；所有统计查询均来自 SQLite 预计算或受控交集查询。' : '当前真实任务二数据尚未导入。系统功能已可用，但不会用合成数据冒充实际结果。'" />
    </div>
  </section>
</template>
